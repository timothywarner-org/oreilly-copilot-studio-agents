...
**DO:** Follow [lab section 2](../modules/04-operate/lab.md): Evaluation, New evaluation, Single responses; import or enter E04/E05/E08. The native CSV has `Question` then `Expected response`; download the template and verify imported rows. The repository JSON is a scenario specification. Select Compare meaning and an explicit threshold for semantic answers, and Tool use with the actual GetStudySession capability for E08. For the course, 80% meaning is a starting threshold, not a certification of correctness. Confirm test identity and connections under Additional configuration > Manage. Start the run.

While results arrive, ask learners to classify the three expected failure types: source, premature answer, action. Open E08 and select **Show activity map**. Inspect actual `focus=cloud`, then `status` and `plan`; verify 30 minutes and visible action output. Open a failing rehearsal case and inspect expected/actual answer, grader explanation, and resource nodes. Do not interpret generated rationale as an authoritative record of the model's internal reasoning.

Open the prepared before/after runs of the **same set**, select **Compare with**, and name the single changed instruction or binding. Inspect each case for regression. Export results via **Export test results**. Record invalid/error outcomes separately; never quietly remove them to inflate the pass rate. Native response time is a measurement and does not itself change the native pass rate.
